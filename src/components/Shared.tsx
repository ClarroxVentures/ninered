...
export const Navbar = () => {
  ...
  const handleNavClick = (href: string) => {
    setIsOpen(false);
    // If on home page and clicking anchor, it works automatically.
    // If on other page and clicking anchor, we need to go to home first.
    ...
    <nav className="fixed top-0 left-0 right-0 z-50 bg-[#111111]/90 backdrop-blur-md border-b border-[#ED1C24]/20">
      <div className="max-w-7xl mx-auto px-6 h-20 flex items-center justify-between">
        ...
        <Link to="/" className="flex items-center gap-1">
          <NineredLogo className="h-12 w-auto" lightText={true} />
        </Link>

        {/* Desktop Nav */}
        <div className="hidden md:flex items-center gap-4">
          {['Services', 'Industries', 'Case Studies', 'Contact Us'].map((item) => {
             const href = item === 'Case Studies' ? '/case-studies' : `#${item.toLowerCase().replace(' ', '-')}`;
             return (
                <AnimatedButton 
                key={item} 
                href={href}
    ...
    <footer className="bg-[#0a0a0a] border-t border-white/[0.06] py-8">
      <div className="max-w-7xl mx-auto px-6 flex flex-col md:flex-row items-center justify-between gap-4">
        <div className="flex items-center gap-1">
          <NineredLogo className="h-10 w-auto" lightText={true} />
        </div>
        
        <div className="text-white/30 text-xs">
          © 2025 Ninered Digilab. All rights reserved.
        </div>
        ...
        <div className="flex gap-6">
          {['Privacy Policy', 'Terms', 'Contact'].map((item) => (
            <a key={item} href="#" className="text-white/35 hover:text-brand-red text-xs transition-colors">
              {item}
            </a>
          ))}
        </div>
    ...
    <AnimatePresence>
      {isVisible && (
        <motion.button
          ...
          onClick={scrollToTop}
          className="fixed bottom-8 right-8 z-50 p-3 bg-brand-red text-white rounded-full shadow-lg hover:bg-brand-dark-red transition-colors"
